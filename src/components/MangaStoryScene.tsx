...
import LetterComponent from "./LetterComponent"; // ✅ Import the new component
...
const MangaStoryScene = ({ sceneNumber, onNext, isActive, sceneData, isCelebration }: MangaStorySceneProps) => {
  ...
  const [showLetter, setShowLetter] = useState(false); // ✅ New state for letter
  ...
  // ✅ Add function to handle letter display
  ...
  const handleNext = () => {
    ...
    if (currentDialogue < currentScene.characters.length - 1) {
      ...
      // Show letter instead of going to next scene
      setShowLetter(true);
    } else {
      onNext();
    }
  ...
  const handleLetterClose = () => {
    setShowLetter(false);
    onNext(); // Continue to next scene after letter is closed
  };
          ...
          {showButton && (
            ...
            >
              {currentDialogue < currentScene.characters.length - 1 
                ? "Next" 
                : handleLetterScene() 
                  ? "Read Letter 💌" // ✅ Special button text for letter scene
                  : "Continue →"
              }
            </Button>
          )}
